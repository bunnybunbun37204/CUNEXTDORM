...
import {
	...
	A as AstroError,
	a9 as AstroResponseHeadersReassigned,
	a7 as ClientAddressNotAvailable,
	...
	ae as renderEndpoint,
	Z as renderJSX,
	ab as renderPage,
	Y as renderSlotToString,
	b as renderTemplate,
	aa as responseSentSymbol$1,
...
} from "./astro-designed-error-pages_dDkNDAy_.mjs";
...
class AstroSession {
	...
	constructor(cookies, { cookie: cookieConfig = DEFAULT_COOKIE_NAME, ...config }) {
...
class RenderContext {
	constructor(
		...
		props = {},
		partial = void 0,
		session = pipeline.manifest.sessionConfig ? new AstroSession(cookies, pipeline.manifest.sessionConfig) : void 0,
	) {
...
class AppPipeline extends Pipeline {
	...
	static create(
		manifestData,
		{ logger, manifest, runtimeMode, renderers, resolve, serverLike, streaming, defaultRoutes },
	) {
...
class App {
	...
	match(request) {
		...
		const routeData = matchRoute(decodeURI(pathname), this.#manifestData);
	...
	async #renderError(
		request,
		{ locals, status, response: originalResponse, skipMiddleware = false, error, clientAddress },
	) {
	...
	#mergeResponses(newResponse, originalResponse, override) {
		...
		const mergedHeaders = new Map([...Array.from(newResponse.headers), ...Array.from(originalResponse.headers)]);
